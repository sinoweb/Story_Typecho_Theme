 ...
 * @license GNU General Public License 2.0
 */
class Story_Radio extends Typecho_Widget_Helper_Form_Element
{
    ...
    public function input($name = NULL, array $options = NULL)
    {
        foreach ($options as $value => $label) {
            ...
            $item->addItem(new Story_CustomLabel('<label class="mdui-radio">'));
            $item->addItem($this->_options[$value]->setAttribute('name', $this->name)
                ->setAttribute('type', 'radio')
                ->setAttribute('value', $value)
                ->setAttribute('id', $id));

            $item->addItem(new Story_CustomLabel('<i class="mdui-radio-icon"></i>'.$label.'</label>'));
