 ...
 * @license GNU General Public License 2.0
 */
class Story_Checkbox extends Typecho_Widget_Helper_Form_Element
{

    public function start(){}

    public function end(){echo '</ul></div></div>';}
    ...
    public function __construct($name = NULL, array $options = NULL, $value = NULL, $label = NULL, $description = NULL)
    {
        ...
        $this->addItem(new Story_CustomLabel('
		<div class="mdui-col-xs-12">
		<div class="mdui-card-actions">
		<div class="mdui-card-primary-title">'.$label. '</div>
		<ul class="typecho-option" id="typecho-option-item-'.$name.'-'.self::$uniqueId.'">'));
    ...
    public function input($name = NULL, array $options = NULL)
    {
        foreach ($options as $value => $label) {
            ...
            */
            ...
            $item->addItem(new Story_CustomLabel('<label class="mdui-checkbox">'));
            $item->addItem($this->_options[$value]->setAttribute('name', $this->name . '[]')
                ->setAttribute('type', 'Checkbox')
                ->setAttribute('value', $value)
                ->setAttribute('id', $id));
            $item->addItem(new Story_CustomLabel("<i class=\"mdui-checkbox-icon\"></i>
$label</label>"));

            /*$labelItem = new Story_CustomLabel('<label class="mdui-checkbox"><input name="'.$this->name.'[]" type="Checkbox" value="'.$value.'" id="'.$id.'"/><i
